# Contributing to Digital-Process-Tools/claude-oss
...
Install the development dependencies once, then run the suite:
...
`pytest-cov` is required by `addopts` in `pyproject.toml`, so a bare `pytest` in a
fresh checkout fails before a single test runs (#611). That is a missing dependency,
not a broken suite.

**Do not run the full suite locally to decide whether a change is good.** It takes
minutes on one operating system and one interpreter, and CI answers a stronger
question: 13 legs across 3 operating systems and Python 3.9-3.12, in parallel. This
repository's expensive failures have repeatedly been on the axis a local run cannot
reach. Run your change's own tests plus the guards it touches, push, and let CI
answer the rest.
