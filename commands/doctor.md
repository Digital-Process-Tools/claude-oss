...
hands back one `repaired:`/`not-ours:`/`could-not-tell:`/`could-not-repair:` line per finding
rather than leaving the hunt in this session for good. Relay its report the same way you relayed
the raw diagnostic above.
...
look at. A `--root` that is not a directory is a `FAIL` line, not a crash: the run continues and
every config-dependent check below reports itself unmeasured.
...
- `WARN … names <a>, but doctor.py ran from <b>` — a flag or an environment variable disagreeing
  ...
  checkout inside a session that exported `CLAUDE_PLUGIN_ROOT` pointing at another.
...
Every one of the three ends with the same sentence: **this is one command's copy, and nothing here
says which copy answered any other command or skill in this session** (#248). A command's text is
resolved once, at invocation, and stays in the turn for its whole length — `/reload-plugins` moves
the registry and does not move text already injected. Do not read this line as a session-wide
clearance.
...
- `WARN … SKEW — … differ in K of N compared file(s): …` — they do not, and the line names the
  files and the version that hid it. **This is a report, not a refusal**: disagreement is the normal
  state for the whole window between a merge and a release.
...
(#240). Quote from the clone at a named sha before filing anything about this plugin's own
documents.
...
- `WARN … running under binary translation (host architecture arm64)` — Rosetta 2, or its
  equivalent. The remedy is a native `python3` rather than anything in the repo. It is a
  performance fact, not a fault: nothing is broken and everything is slow.
- `OK … was NOT probed` — no probe is implemented for this platform, so nothing was attempted
  and nothing will be until somebody writes one. **Do not read this as native.**
  `platform.machine()` cannot answer the question and neither can `uname -m` from a subprocess: an
  emulated process is shown the *emulated* architecture and so is everything it spawns. The probe
  used on macOS is the `sysctl.proc_translated` flag, read in-process; Linux (qemu-user) and
...
A gap that is **unobservable in principle here** is named on an `OK` line and does not spend the
warning count — the shape `agent_dispatch` already uses in this file. A probe that **exists and
failed** keeps its `WARN`. Neither line ever contains the word *native*, and a test pins that.

**`cpu topology`** — the logical core count, split into performance and efficiency cores where
the platform exposes it (macOS does; the line says so explicitly when it does not). Three states
of its own, and the third is separate on purpose: `OK … N logical core(s) -- P performance + E
efficiency`; `OK … reports no performance/efficiency core split`; and `WARN … whether they are
split … could NOT be determined`, when the `hw.nperflevels` probe ran and did not answer. The
second and third are one sentence apart and mean opposite things — the second says the count below
sizes against uniform cores, the third says nobody established that. `WARN` too when no core count
...
things it is there to say. The cap exists and is read before any core is counted; and on a machine
with an efficiency-core split the number exceeds the performance cores, while **several agents
running suites concurrently each size against the whole machine without seeing each other** — the
doctor cannot count the other agents, and does not pretend to. `WARN` when the count is unknown;
the count is never invented as 0 or 1.
...
`commands/`, `skills/` and `agents/` text names (#582).
...
The expected set is derived from the shipped text at call time, never listed beside the check.
It has two paths into it, not one (#739): call-shaped derivations across `commands/`, `skills/`
and `agents/`, plus `SKILL.md`'s own op table — written with the op *as* the cell, never preceded
by the word `supertool` — parsed by its own heading rather than swept as free text. A bare
op-shaped word outside the table is not derived.
...
one fleet while each board renders as its own.
...
- `WARN … binds the SHARED default socket` — nothing declared, nothing exported, and nothing to
  derive from. One process holds that socket, the first one wins, and the loser is never told. The
  line names which of six reasons applied — no `.oss.json`, one this process could not read, one
  that read and parsed and is not an object, one with no `repo`, one whose `repo` the validator
  refuses, or a validator that would not import — because the remedies differ. The second and third
  are separate (#216): a mode and a shape are not the same errand.
- `OK … so this is the export bin/oss-workspace makes for this repo` — an export with no
  declaration beside it, and it is **exactly** what `.oss.json`'s `repo` derives to. This is the
  ordinary state of every managed repo (#192).
- `WARN … it is not what .oss.json's repo derives to` — earned by a comparison rather than inferred
  from an absence (#150). A hand-copied `.claude/settings.local.json` puts one name into several
  repos, and each of them then reports a fleet that is not its own.
- `WARN … whether this is the export bin/oss-workspace derives … is unknown` — an export is set and
  ...
  and not as derived, which would clear on none. The line names which of the six it was, and
  "could not read it" and "read it, and it is the wrong shape" are two of them (#216).
...
- `WARN … .supertool.json is not an object` / `` `ops` in .supertool.json is not an object`` — the
  ...
  above (#216): two failure states exist because they have different remedies.
...
socket. Two channel-capable servers would satisfy both halves apart."* The diagnostic declines the
question rather than guessing at it — answering it needs `lsof`, `ps`, a walk up a process tree and
the harness's own server name, none of which exist on every platform — and it prints the decline
...
session — a `claude mcp` registration pointing at `claude-channel/channel.ts` (#621). This line
asks that where a maintainer running the diagnostic can see it.
...
- `OK … is registered pointing at … which exists` — the registration is real and the file it names
  ...
  that `bun` is on PATH, or that anything is listening on the socket.
...
path is pinned to against the version supertool is actually installed at (#646). This line only runs
when the registration line above resolved to `registered`; every other registration state already
has nothing to compare. It asks the registration a second time rather than reusing the answer above,
so each check stays testable and stubbable on its own: two real `claude mcp get` calls per run, not
one.
...
(`notifiers/claude-channel/channel.ts`). Two servers racing that script for one Unix socket is
invisible from inside a session: one binds, the harness's connection to the other is refused, and
`channel:health` can only report `CANNOT DETERMINE` (#810). `bin/oss-workspace` runs the identical
check, via the identical function, before deciding whether to arm
`--dangerously-load-development-channels` -- and when it runs first (the ordinary launcher path),
it relays its own answer into this diagnostic's environment (`OSS_WORKSPACE_CENSUS_CHECKED`,
`_REPORT`), rather than this line asking `claude mcp list` a second time seconds later. Run
standalone (`/oss:doctor` on its own, outside a launcher-opened session), no relay is present and
this line asks for itself.
...
channel-capable MCP server racing this repo's socket? (#860) This line is that comparison.
...
This runs on every invocation rather than behind a flag or a slower clock.
...
the matcher find these rows*; this one answers *does anything look in that directory at all*
(#119).
...
dependency's own test fixtures satisfies nothing (#241). Five things it can say:
...
- **`… enumerates layers by directory glob rather than by naming them in a fixed list …`**, at `OK`
  ...
  construction. This is the state the upstream fix produces. The glob still has to be found in a
  hook the runtime executes, and one in a test fixture answers nothing. What it does not establish,
  and says so: what the loop's body then does with each directory it visits. A **filter** narrowing
  the glob to literal layer names, none of them ours, sends this back to `WARN … could not be
  determined` rather than to a gap — a filter this cannot see the whole of is a reason to withhold
...
- **`… could not be determined …`**, at `WARN` — nothing was measured, and **the line always names the
  ...
  test fixture is evidence about the fixture and about nothing else. The no-fixed-list one is the
  expected shape of the upstream fix, so it too is *unknown* rather than a pass **or** a gap.
...
explain which directory is already owned, while its real remedy is a `languages:` config change;
and the fragments-README gap explicitly warns `/oss:scaffold will NOT fix it`. A looser match on
the bare substring would misread that last WARN as pointing at the very command it refuses, and
would send the CodeQL WARN down the wrong remedy entirely. Those other scaffold-adjacent WARNs are
read and acted on by hand, the same as any other `WARN` this derivation does not classify.

The diagnostic itself never names a next command: a line printed regardless of state carries no
information, and the advice belongs on the surface that instructs the work. `scripts/doctor.py` is
also called directly, script to script, from inside a running tick (`commands/tick.md`) and before
a release (`skills/manager/SKILL.md`) -- neither of those callers reads this section.
