...
#: already is (#1302): the directory itself is a DEFAULT -- created once when absent, then the
#: repo's own forever, and every fragment inside it is theirs, never touched -- while this one
#: file inside it is OURS, replaced wholesale on every run. It is not under `OWNED_DIR`
#: (`.oss/`) because `.oss/` is a directory this plugin owns outright; `trap.d/` is not, and
#: mixing this file into `.oss/` instead would mean nothing marks the directory it actually
#: documents.
TRAP_DIR = "trap.d"

TRAP_README = """# trap.d/ — one fragment per lesson, no ceremony
...
def _render_trap_readme(config, plugin_root):
    return TRAP_README
...
OWNED = {
    ...
    # Not gated in CHANGELOG_OWNED below (#1302): a changelog gate already running under
