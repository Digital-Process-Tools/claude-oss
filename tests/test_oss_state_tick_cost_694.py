...
def test_a_first_tick_own_start_ctx_is_its_own_floor():
    ...
    record = oss_state.tick_cost(
        SESSION, WINDOW, start_ctx=96000, calls=12, context_carried=180000, is_first=True
    )
...
def test_a_later_tick_derives_inherited_from_the_propagated_floor():
    ...
    record = oss_state.tick_cost(
        SESSION, WINDOW, start_ctx=411000, calls=9, context_carried=2_000_000,
        is_first=False, prior_floor=49000,
    )
    assert record["state"] == oss_state.TICK_COST_MEASURED
...
def test_a_non_first_tick_with_no_known_floor_is_floor_unknown_not_zero_and_not_start_ctx():
    ...
    record = oss_state.tick_cost(
        SESSION, WINDOW, start_ctx=345000, calls=6, context_carried=2_100_000,
        is_first=False, prior_floor=None,
    )
...
def test_could_not_measure_needs_a_why():
    with pytest.raises(oss_state.StateError) as caught:
        oss_state.tick_cost(SESSION, WINDOW, start_ctx=None, calls=6, context_carried=100)
...
def test_a_partial_reading_still_reports_the_pieces_that_were_taken():
    ...
    record = oss_state.tick_cost(
        SESSION, WINDOW, start_ctx=None, calls=5, context_carried=100000,
...
def test_first_tick_asserted_against_a_session_that_already_has_a_floor_is_refused():
    ...
    with pytest.raises(oss_state.StateError) as caught:
        oss_state.tick_cost(
            SESSION, WINDOW, start_ctx=500000, calls=1, context_carried=10,
            is_first=True, prior_floor=49000,
        )
...
def test_no_rate_means_no_cost_is_claimed():
    record = oss_state.tick_cost(
        SESSION, WINDOW, start_ctx=96000, calls=1, context_carried=2_000_000, is_first=True
    )
...
def test_a_rate_produces_a_cost_labelled_list_rate_not_a_bill():
    ...
    might skip."""
    record = oss_state.tick_cost(
        SESSION, WINDOW, start_ctx=96000, calls=1, context_carried=2_000_000,
        is_first=True, rate=1.5,
    )
...
def test_cost_is_computed_even_when_the_floor_is_unknown():
    ...
    record = oss_state.tick_cost(
        SESSION, WINDOW, start_ctx=345000, calls=6, context_carried=2_100_000,
        is_first=False, prior_floor=None, rate=1.5,
    )
    assert record["state"] == oss_state.TICK_COST_FLOOR_UNKNOWN
...
TICK_COST_ARGV = [
    "--at", STAMP,
    "--tick-cost-session", SESSION,
    "--tick-cost-window", WINDOW,
    "--tick-cost-start-ctx", "96000",
    "--tick-cost-calls", "5",
    "--tick-cost-context-carried", "180000",
    "--tick-cost-first",
]
...
def test_cli_propagates_the_floor_to_a_later_tick_in_the_same_session(tmp_path):
    ...
    later_argv = [
        ...
        "--tick-cost-session", SESSION,
        "--tick-cost-window", WINDOW,
        "--tick-cost-start-ctx", "411000",
        "--tick-cost-calls", "9",
        "--tick-cost-context-carried", "2000000",
    ]
...
def test_cli_a_different_sessions_tick_never_sees_this_sessions_floor(tmp_path):
    ...
    other_session_argv = [
        ...
        "--tick-cost-session", "a-different-session",
        "--tick-cost-window", WINDOW,
        "--tick-cost-start-ctx", "50000",
        "--tick-cost-calls", "3",
        "--tick-cost-context-carried", "10000",
    ]
...
def test_cli_reports_unknown_via_the_unknown_literal_not_a_guessed_number(tmp_path):
    path = tmp_path / "state.json"
    argv = [
        str(path), "--decision", "no usage data this turn", "--at", STAMP,
        "--tick-cost-session", SESSION,
        "--tick-cost-window", WINDOW,
        "--tick-cost-start-ctx", "unknown",
        "--tick-cost-calls", "unknown",
        "--tick-cost-context-carried", "unknown",
        "--tick-cost-why", "the transcript truncated before a usage block appeared",
    ]
    result = _piped(argv)
    assert result.returncode == 0, result.stdout
...
def test_a_refused_decision_drops_nothing_silently(tmp_path):
    ...
    result = _piped([str(path), "--decision", long_decision, "--at", STAMP] + TICK_COST_ARGV)
    assert result.returncode == 1
    assert "FAIL" in result.stdout
