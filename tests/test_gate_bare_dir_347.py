...
def test_a_bare_dir_across_a_newline_does_not_capture_the_next_flag(tmp_path):
    ...
    assert detail
    assert (
        "--changelog" not in detail
        or "no argument" in detail.lower()
        or ("argument" in detail.lower())
    )
...
def test_a_well_formed_single_line_dir_still_parses(tmp_path):
    ...
    assert oss_config.scaffolded_changelog_gate(root) == (
        "present-other-dir",
        "docs/frags",
    )
...
def test_multiline_run_block_with_a_well_formed_dir_on_its_own_line_still_parses(
    tmp_path,
):
