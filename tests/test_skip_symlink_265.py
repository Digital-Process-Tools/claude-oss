...
def test_junction_answers_not_windows_off_windows(tmp_path):
    ...
    if sys.platform == "win32":
        pytest.skip(
            "this machine is win32 -- the 'not windows' branch does not apply here"
        )
...
def test_a_directory_case_names_both_mechanisms_when_both_fail(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        skip_symlink,
        "_junction",
        lambda link, target: (False, "spy: junction declined"),
    )
    link = tmp_path / "evil"
    ...
    if landed:
        ...
        try:
            ...
            pytest.fail(
                ...
                "a successful fallback must be asserted, not silently skipped".format(
                    exc
                )
            )
        assert result == link
...
def test_a_file_case_names_only_the_symlink_mechanism_when_it_fails(
    ...
    with pytest.raises(pytest.skip.Exception) as excinfo:
        skip_symlink.symlink_or_skip(
            link, target, target_is_directory=False, what="the probe"
        )
    reason = str(excinfo.value)
    assert "symlink_to raised" in reason
...
def test_the_directory_case_actually_calls_junction_when_symlink_to_fails(
    tmp_path, monkeypatch
):
...
def test_a_working_symlink_is_returned_and_resolves_to_target(tmp_path):
    ...
    try:
        ...
        pytest.skip(
            "this platform would not create a plain symlink either: {}".format(exc)
        )
    assert result == link
    assert link.resolve() == target.resolve()
