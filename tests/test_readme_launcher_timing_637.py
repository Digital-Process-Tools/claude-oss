"""README's launcher-timing receipt names a method and a date (#637).

`README.md` states, as a measurement, how much the launcher's `/oss:doctor`
diagnostic costs to run at session-open. The figures went stale silently
once before this issue: they were taken before #621 added a `claude mcp get`
subprocess to every doctor run, and the sentence did not move when that
