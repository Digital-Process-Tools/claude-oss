...
def test_owned_files_live_in_one_directory():
    ...
    one rule, and a reader can see the boundary without opening anything -- with three
    ...
    README.md` (#1302) and `outbound/README.md` (#1395) cannot either, because `.oss/`
    is a directory this plugin owns outright and neither `trap.d/` nor `outbound/` is
    -- each is a DEFAULT whose one owned file this marks.
    """
