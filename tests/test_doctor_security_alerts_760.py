...
def test_could_not_tell_when_gh_is_not_on_path(tmp_path, monkeypatch):
    monkeypatch.setattr(
        doctor_check_security_alerts.gh_which,
        "safe_which",
        lambda name, path=None: None,
    )
    state, detail = doctor.security_alert_state(
        tmp_path, "code-scanning", config=_config()
    )
...
def test_200_is_configured_for_every_scanner(tmp_path, scanner):
    ...
    run = _run_once(0, "[]", "")
    state, detail = doctor.security_alert_state(
        tmp_path, scanner, config=_config(), run=run
    )
...
def test_404_no_analysis_found_is_never_scanned(tmp_path):
    run = _run_once(
        1, '{"message": "no analysis found"}', "gh: no analysis found (HTTP 404)"
    )
    state, detail = doctor.security_alert_state(
        tmp_path, "code-scanning", config=_config(), run=run
    )
...
def test_404_secret_scanning_disabled_body_is_disabled_not_never_scanned(tmp_path):
    ...
    state, detail = doctor.security_alert_state(
        tmp_path, "secret-scanning", config=_config(), run=run
    )
...
def test_404_dependabot_disabled_body_is_disabled(tmp_path):
    ...
    state, detail = doctor.security_alert_state(
        tmp_path, "dependabot", config=_config(), run=run
    )
    assert state == "disabled"
...
def test_check_dependabot_alerts_reports_warn_with_remedy_when_never_scanned(
    tmp_path, capsys
):
    run = _run_once(
        1, '{"message": "no analysis found"}', "gh: no analysis found (HTTP 404)"
...
def test_check_secret_scanning_alerts_reports_warn_when_disabled(tmp_path, capsys):
    ...
    doctor.check_secret_scanning_alerts(
...
def test_remedy_link_uses_the_origin_fallback_slug_not_only_config(
    ...
    monkeypatch.setattr(
        doctor,
        "_origin_slug",
        lambda project_dir, run=None: ("owner/from-origin", None),
    )
    run = _run_once(
        1, '{"message": "no analysis found"}', "gh: no analysis found (HTTP 404)"
    )
...
def test_check_reports_could_not_tell_distinctly_never_as_ok_or_a_count(
    tmp_path, capsys
):
...
def test_code_scanning_warn_carries_the_default_setup_command(tmp_path, capsys):
    ...
    doctor.check_code_scanning_alerts(
        tmp_path, config=_config(repo="owner/name"), run=run
    )
    out = capsys.readouterr().out
    assert doctor.FINDINGS[-1][0] == "WARN"
    assert (
        "gh api -X PATCH repos/owner/name/code-scanning/default-setup -f state=configured"
        in out
    )
...
def test_secret_scanning_warn_carries_the_security_and_analysis_command(
    tmp_path, capsys
):
    run = _run_once(
        1, '{"message": "no analysis found"}', "gh: no analysis found (HTTP 404)"
