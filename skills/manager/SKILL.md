...
| Before | Call |
| --- | --- |
| naming a lane, or bundling a second issue into one | `scripts/lane_setup.py <issue> --lane PATTERN --derive-held` (fallback: `--against PATTERN`, only on `could-not-derive-the-held-set`) |
