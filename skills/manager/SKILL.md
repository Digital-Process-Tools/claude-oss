...
| Filing | `gh-issue-create:@FILE` — the payload carries `labels.filed_by_loop`'s label only for a filing the loop made on its own initiative, never for one co-decided with the maintainer in session (#762, #798, #1083), plus `labels.priority` and `labels.lane_other` so the issue is not invisible to dispatch until the next triage sweep (#1682) |
