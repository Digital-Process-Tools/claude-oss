...
- The two-round audit cap no longer quietly outranks the ranking table. Gate 3 and
  `agents/release-auditor.md` both ended round two by filing whatever remained and shipping over it,
  while the table said some classes block unconditionally; the later sentence won. A finding in a
  blocking row is now not carry-forward material and stops the tag in either round (#83).
