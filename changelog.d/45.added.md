- `/oss:setup` now settles the merge permission, and `/oss:doctor` reports whether it is settled.
  ...
  three states and not two: a rule naming the op is present, none is, or the settings files could
  not be read. Neither the command nor the check writes a permission grant: arranging an
  irreversible action without asking is not this tool's job. The passing state says what it measured
