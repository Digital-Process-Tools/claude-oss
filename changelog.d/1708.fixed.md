- `oss:doctor`'s priority-label family check no longer reads a doctor-created
  ...
  labels that call actually created rather than always claiming all three,
  and the `WARN` lines no longer claim "no priority-* labels exist" when a
  partial family is what is actually there (#1708).
