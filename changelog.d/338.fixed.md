- The developer brief now states the `pr_body.closes` duty that version 4 of the report schema made
  ...
  closing would have been ceremony. The fragment is keyed to #334 because that is the real number
  this change hangs off and no number of its own existed when it was written; whoever opens the pull
  request should rename it to that number, which is what the one-file-per-pull-request convention
  in `changelog.d/README.md` asks for.
