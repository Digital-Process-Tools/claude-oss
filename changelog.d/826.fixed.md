- `dispatch_rank._band` used to read an unrecognised priority label -- a
  ...
  the board. A label sharing no such prefix, and a board whose declared
  spellings share no prefix at all, are both left alone -- there is no
  signal to read the second case from, and inventing one would be exactly
  the hardcoded-fact failure this module already refuses elsewhere (#826).
