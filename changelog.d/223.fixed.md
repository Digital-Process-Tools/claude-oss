- A filename in the managed repository's own `01-oss` rule layer can no longer forge a row of
  ...
  three print statements put those rows on stdout without flattening, so a file whose name carries a
  ...
  composition. Fixed at the chokepoint rather than at the three prints: the repository-derived name
  is flattened at the one place it enters `plan_rules`' `entries`, so every consumer — the three
  prints, `--show`, `doctor` — inherits a single-line `path` without having to remember, and the
  fifth print statement added later is not in the position the third one was. Flattened after the
