- `/oss:doctor` now reports two facts about the machine it is running on that nothing printed
  ...
  **not probed** and says so rather than folding into `native`, which would clear an emulated
  interpreter on exactly the platforms nobody checked -- at the cost, weighed rather than
  overlooked, of a standing `WARN` on those platforms that no remedy clears, so their verdict
  reads `usable with gaps` where it would otherwise read `ok`. The host architecture and the
